...
function App() {
  ...
  const artwork1Percentage = totalVotes > 0 ? (votes[1] / totalVotes) * 100 : 50
  const artwork2Percentage = totalVotes > 0 ? (votes[2] / totalVotes) * 100 : 50
      ...
      <header className="border-b border-white/10 backdrop-blur-sm bg-black/20">
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white mb-2">AI Art Battle Arena</h1>
              <h2 className="text-lg text-gray-300 mb-1">Real-Time AI Art Voting Competition</h2>
              <p className="text-gray-400">Vote for your favorite AI-generated artwork based on trending news topics</p>
            </div>
              ...
              >
                <Card className={`relative overflow-hidden border-2 transition-all duration-300 ${
                  ...
                  <CardContent className="p-0">
                    ...
                    <div className="p-6">
                      <h3 id={`artwork-${artwork.id}-title`} className="text-xl font-bold text-white mb-3">{artwork.title}</h3>
                      
                      <div className="mb-4" aria-label={`Voting statistics for ${artwork.title}`}>
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-gray-300">Votes: {votes[artwork.id]}</span>
                          <span className="text-gray-300">
                            {artwork.id === 1 ? artwork1Percentage.toFixed(1) : artwork2Percentage.toFixed(1)}%
                          </span>
                        </div>
                        <Progress 
                          value={artwork.id === 1 ? artwork1Percentage : artwork2Percentage} 
                          className="h-2"
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-semibold">#{roundNumber}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-white/20 bg-black/40 backdrop-blur-sm">
              <CardContent className="p-6">
                ...
                <div className="space-y-2" role="log" aria-live="polite" aria-label="Recent voting activity">
                  <AnimatePresence>
                    {recentVoters.map((voter, index) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="text-xl" role="img" aria-label="Country flag">{voter.country}</span>
      ...
      <footer className="border-t border-white/10 mt-16">
        <div className="container mx-auto px-4 py-6">
          <p className="text-center text-gray-400 text-sm">
            AI Art Battle Arena - Where creativity meets competition. Powered by artificial intelligence and human creativity.
          </p>
        </div>
      </footer>
    </div>
